...
class SAM3D:
    ...
    def init_mask_with_points(self, image, points):
        # We must supplz both point_coords and point_labels
        # points example from the other repo
        points = torch.tensor(points, device=self.device).reshape(-1, 1, 2)
        labels = torch.ones(len(points), device=self.device).reshape(-1, 1)
        transformed_coords = self.predictor.transform.apply_coords_torch(points, image.shape[:2])
        masks, scores, logits = self.predictor.predict_torch(
            point_coords=transformed_coords,
            point_labels=labels,  # all points are considered foreground
            boxes=None,
            multimask_output=False,
        )

        # Here render the mask and the input image
        best_mask = masks[0].cpu().numpy()
        best_mask = best_mask[0][..., None] #[H, W, 1]

        # Show the initial image with point and mask
        plt.subplots(1, 2)
        plt.subplot(1, 2, 1)
        plt.imshow(image)
        plt.scatter(points[:, 0, 0].cpu().numpy(), points[:, 0, 1].cpu().numpy(), c='r', s=10)
        plt.subplot(1, 2, 2)
