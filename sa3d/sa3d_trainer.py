...
from sa3d.sa3d_pipeline import SA3DSeathruPipelineConfig
...
class SA3DTrainer(Trainer):
    ...
    def save_visualzation(self):
        for k in self.vis_variables:
            ...
            # Pad if height not divisible by 2
            if self.train_vis[k][0].shape[0] % 2 != 0:
                # Pad every frame with zeros at the bottom
                self.train_vis[k] = np.pad(self.train_vis[k], ((0,0), (0, 1), (0, 0), (0,0)), mode="edge")
                print(self.train_vis[k].shape)

            save_dir = Path(self.base_dir / f"train_vis")
            if not save_dir.exists():
                save_dir.mkdir(parents=True, exist_ok=True)
            imageio.mimwrite(save_dir / f"{k}.mp4", self.train_vis[k], fps=30, codec="libx264")
